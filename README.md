...
3. You can then access the Pi via SSH (with software like <b>Putty</b> on Windows e.g.) and update/upgrade your system with `sudo apt-get update` and `sudo apt-get upgrade -y`. Ultimately, reboot with `sudo reboot`.
...
2. Change the automatically generated password to access the web interface of the piHole with `sudo pihole setpassword`. After that, `reboot`.
3. Set up your router to use the piHole's IP as its DNS server. In some cases you also need to restart the router.
4. At this point I like to make my piHole's IP static from the raspberry itself by using Network Manager TUI (`sudo nmtui`). Open it, go through the menus (<i>Edit a connection</i> --> <i>Wired Connection</i>) and edit the <i>IPv4 CONFIGURATION</i> by switching from automatic to manual with the following values:
```
Adresses:    <static IP of the Raspberry Pi>
Gateway:     <IP of the router>
DNS servers: <static IP of the Raspberry Pi>
```
Exit via <i>OK</i> --> <i>Back</i> --> <i>Quit</i>.
6. Access the piHole's web interface within your browser by entering `<IP of Raspberry Pi>:<port of your piHole WebUI>/admin` and configure the available settings to your liking. There are several guides out there to help with configuring piHole properly. A nice selection of adlists can be found on [Hagezi's GitHub](https://github.com/hagezi/dns-blocklists).
...
1. First, get the PiTFT ready by installing the python modules in a virtual environment with
```
...
```
2. Execute the initialization of the 3.5" 480x320 display with:
```
...
```
6. Lastly, in order to avoid getting a password prompt upon booting, make your user part of the `pihole` group by executing `sudo usermod -aG pihole <yourusername>` 
...
2. Execute the automatic install script for piVPN with `curl -L https://install.pivpn.io | bash` and go through the installation process for the WireGuard VPN server. Reboot when finished.
3. Add users for the piVPN service with `pivpn add`. One user for each client you want to use the VPN with.
7. Import the config files for said users either by either
  + QR code with `pivpn -qr` for devices with integrated cameras or
  + Config file, which you can retrieve via SCP (with software like <b>WinSCP</b> or <b>FileZilla</b>, for example) from `home/<user>/configs` on your Raspberry Pi.
